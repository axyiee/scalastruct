...
trait UnsafePointerFactory:
  ...
  /** Reallocates a value at a specific memory [[address]] with the given [[data]].
    ...
    */
  def realloc[A <: Primitive](address: Pointer, data: A)(using PrimitiveMemoryFactory[A]): Unit

  /** Reallocates the [[length]] of a structure at a specific memory [[address]].
    * @param address
    *   The memory address to be reallocated.
    * @param length
  ...
  /** Access the [[Primitive]] value with the given [[offset]] of the given [[address]].
    ...
    */
  def visit[A <: Primitive](address: Pointer)(using PrimitiveMemoryFactory[A]): A
  ...
  extension (pointer: Pointer)
    ...
    /** Access the [[Byte]] value with the given [[offset]] of this address.
      */
    def access[A <: Primitive](using PrimitiveMemoryFactory[A]): A = this.visit(pointer)
